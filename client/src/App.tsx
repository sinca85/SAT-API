...
function AIKnowledgePanel({ canManage }: { canManage: boolean }) {
  ...
  const openEdit = async (item: any) => { setEditing({ ...item }); try { const data = await requestJson<{ documents: any[] }>(`/admin/ai/documents/${item._id}`); setDocuments(data.documents); } catch { setDocuments([]); } };
