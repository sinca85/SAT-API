...
function AIKnowledgePanel({ canManage }: { canManage: boolean }) {
  ...
  const openEdit = async (item: any) => { setEditing({ ...item }); try { const [documentsData, diagnosticsData] = await Promise.all([requestJson<{ documents: any[] }>(`/admin/ai/documents/${item._id}`), requestJson<{ queries: any[] }>(`/admin/ai/configurations/${item._id}/diagnostics`)]); setDocuments(documentsData.documents); setDiagnostics(diagnosticsData.queries); } catch { setDocuments([]); setDiagnostics([]); } };
