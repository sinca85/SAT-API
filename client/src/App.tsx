...
function AIKnowledgePanel({ canManage }: { canManage: boolean }) {
  ...
  const upload = async (item: any, file: File) => { const body = new FormData(); body.append("file", file); const response = await fetch(`/admin/ai/configurations/${item._id}/documents`, { method: "POST", credentials: "same-origin", body }); if (!response.ok) throw new Error("No se pudo cargar el PDF"); await load(); message.success("PDF cargado y procesado"); };
  return <Space direction="vertical" size="large" style={{ width: "100%" }}>
    ...
    <Table loading={loading} rowKey="_id" dataSource={items} columns={[{ title: "Asistente", render: (_: unknown, x: any) => <Space direction="vertical" size={0}><Typography.Text strong>{x.name}</Typography.Text><Typography.Text type="secondary">/{x.slug}</Typography.Text></Space> }, { title: "Aseguradora", dataIndex: "company" }, { title: "Producto", dataIndex: "product" }, { title: "Estado", dataIndex: "active", render: (v: boolean) => <Tag color={v ? "green" : "default"}>{v ? "Activo" : "Inactivo"}</Tag> }, { title: "Versión", dataIndex: "knowledgeVersion" }, { title: "Acciones", render: (_: unknown, x: any) => <Space wrap>{canManage && <><Button size="small" icon={<EditOutlined />} onClick={() => setEditing({ ...x })}>Editar</Button><Button size="small" icon={<UploadOutlined />} onClick={() => { const input = document.createElement("input"); input.type = "file"; input.accept = "application/pdf,.pdf"; input.onchange = () => { const file = input.files?.[0]; if (file) void upload(x, file).catch(e => message.error(e instanceof Error ? e.message : "Error")); }; input.click(); }}>Adjuntar PDF</Button><Button danger size="small" icon={<DeleteOutlined />} onClick={() => remove(x)}>Eliminar</Button></>}</Space> }]} locale={{ emptyText: <Empty description="Todavía no hay asistentes de IA" /> }} />
