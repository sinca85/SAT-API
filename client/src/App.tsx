...
function LeadsTable({ canManage, canDelete }: { canManage: boolean; canDelete: boolean }) {
  ...
  const columns: TableColumnsType<LeadContactRow> = [
    ...
    { title: "Cotizaciones", key: "quotes", width: 160, render: (_, lead) => lead.quotes.length > 1 ? <Button type="link" size="small" onClick={() => setExpandedContactKeys((current) => current.includes(lead.contactKey) ? current.filter((key) => key !== lead.contactKey) : [...current, lead.contactKey])}>Ver {lead.quotes.length} cotizaciones</Button> : <Typography.Text type="secondary">1 cotización</Typography.Text> },
  ];
...
function AIKnowledgePanel({ canManage }: { canManage: boolean }) {
  ...
  const [loading, setLoading] = useState(true);
  const [draft, setDraft] = useState({ name: "", slug: "", company: "Allianz", product: "Hogar", title: "", fallbackMessage: "No encontré esa información en la documentación disponible.", outOfScopeMessage: "Puedo ayudarte únicamente con consultas sobre el seguro de hogar." });
  ...
  return <Space direction="vertical" size="large" style={{ width: "100%" }}>
    ...
    <Modal open={Boolean(editing)} title="Editar asistente" okText="Guardar" cancelText="Cancelar" onCancel={() => setEditing(null)} onOk={async () => { if (!editing) return; try { await requestJson(`/admin/ai/configurations/${editing._id}`, { method: "PATCH", body: JSON.stringify({ name: editing.name, slug: editing.slug, company: editing.company, product: editing.product, title: editing.title, active: editing.active, fallbackMessage: editing.fallbackMessage, outOfScopeMessage: editing.outOfScopeMessage }) }); setEditing(null); await load(); message.success("Asistente actualizado"); } catch (e) { message.error(e instanceof Error ? e.message : "No se pudo actualizar"); } }}>
