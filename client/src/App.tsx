...
function AIKnowledgePanel({ canManage }: { canManage: boolean }) {
  ...
  const upload = async (item: any, files: File[]) => { await Promise.all(files.map(async (file) => { const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_"); await uploadToBlob(`ai-documents/${item._id}/${safeName}`, file, { access: "private", handleUploadUrl: "/admin/ai/blob-upload", clientPayload: JSON.stringify({ configurationId: item._id, originalName: file.name, sizeBytes: file.size }), multipart: file.size > 5 * 1024 * 1024 }); })); message.success("Archivo subido. Estamos procesando su contenido; actualizá la lista en unos instantes."); window.setTimeout(() => void load(), 3500); };
