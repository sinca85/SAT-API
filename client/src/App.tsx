...
function AIKnowledgePanel({ canManage }: { canManage: boolean }) {
  ...
  const upload = async (item: any, files: File[]) => { setUploadState("Solicitando autorización de Vercel Blob..."); try { const results = await Promise.all(files.map(async (file) => { const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_"); const pathname = `ai-documents/${item._id}/${safeName}`; setUploadState(`Subiendo ${file.name} al almacenamiento privado...`); const blob = await uploadToBlob(pathname, file, { access: "private", handleUploadUrl: "/admin/ai/blob-upload", multipart: file.size > 5 * 1024 * 1024, onUploadProgress: ({ percentage }) => setUploadState(`Subiendo ${file.name}: ${Math.round(percentage)}%`) }); setUploadState(`Procesando ${file.name}...`); const processed = await requestJson<{ document: { status: string; error?: string } }>(`/admin/ai/configurations/${item._id}/documents-from-blob`, { method: "POST", body: JSON.stringify({ url: blob.url, pathname: blob.pathname, originalName: file.name, sizeBytes: file.size }) }); return { name: file.name, ...processed.document }; })); await load(); const failed = results.filter(document => document.status !== "ready"); if (failed.length) throw new Error(failed.map(document => `${document.name}: ${document.error || "error de procesamiento"}`).join(" | ")); setUploadState("Carga completada."); message.success(`${files.length} PDF${files.length === 1 ? "" : "s"} cargado${files.length === 1 ? "" : "s"}`); } catch (error) { const detail = error instanceof Error ? error.message : "Error de transferencia desconocido"; setUploadState(`Error: ${detail}`); throw error; } };
