...
function AIKnowledgePanel({ canManage }: { canManage: boolean }) {
  ...
  const upload = async (item: any, files: File[]) => { const results = await Promise.all(files.map(async (file) => { const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_"); const pathname = `ai-documents/${item._id}/${safeName}`; const token = await requestJson<{ clientToken: string }>("/admin/ai/blob-upload", { method: "POST", body: JSON.stringify({ type: "blob.generate-client-token", payload: { pathname, clientPayload: null, multipart: file.size > 5 * 1024 * 1024 } }) }); const blob = await putToBlob(pathname, file, { access: "private", token: token.clientToken, multipart: file.size > 5 * 1024 * 1024 }); const processed = await requestJson<{ document: { status: string; error?: string } }>(`/admin/ai/configurations/${item._id}/documents-from-blob`, { method: "POST", body: JSON.stringify({ url: blob.url, pathname: blob.pathname, originalName: file.name, sizeBytes: file.size }) }); return { name: file.name, ...processed.document }; })); await load(); const failed = results.filter(document => document.status !== "ready"); if (failed.length) throw new Error(failed.map(document => `${document.name}: ${document.error || "error de procesamiento"}`).join(" | ")); message.success(`${files.length} PDF${files.length === 1 ? "" : "s"} cargado${files.length === 1 ? "" : "s"}`); };
