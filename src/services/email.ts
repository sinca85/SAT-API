...
async function commercialWhatsAppUrl() {
  const entry = await SiteConfig.findOne({ type: "whatsapp", active: true }).select("value").lean();
  const number = entry?.value?.replace(/\D/g, "") || "";
...
export async function sendHomeQuoteEmail(input: { name: string; email: string; homeType: string; quote: HomeQuote }) {
  ...
  const whatsappUrl = await commercialWhatsAppUrl();
  const contactHtml = whatsappUrl
    ? `Respondé este mail o <a href="${whatsappUrl}" style="color:#075aca;font-weight:700">escribinos por WhatsApp hoy mismo</a>.`
    : "Respondé este mail para que un asesor pueda ayudarte hoy mismo.";
  const contactText = whatsappUrl
    ? `Respondé este mail o escribinos por WhatsApp hoy mismo: ${whatsappUrl}`
    : "Respondé este mail para que un asesor pueda ayudarte hoy mismo.";
